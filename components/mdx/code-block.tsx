...
export function CodeBlock({ children, raw, ...props }: CodeBlockProps) {
  return (
    <>
      ...
      <pre
        className="relative mb-4 mt-6 max-h-[640px] overflow-x-auto rounded-lg border border-muted bg-muted p-4 font-mono text-sm font-semibold text-muted-foreground"
        {...props}
      >
        {children}
      </pre>
