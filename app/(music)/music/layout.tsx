          ...
          <div className="pt-2 shrink-0">
            ...
            <div className="items-center text-xs md:text-sm text-texthigh hover:text-foreground cursor-pointer">
              ...
              {isSearchBarVisible ? (
                ...
                >
                  <div onClick={toggleSearchBar} className="flex items-center">
...
{isSidebarOpen ? (
    <div
        className="pb-1 px-4 h-[24px] ml-1 items-center text-xs font-medium"
    >
        Melody Nexus
    </div>
) : (
    ...
    <div
                ...
                className={`items-center text-xs md:text-sm text-texthigh hover:text-foreground cursor-pointer ${
                  activeContent === "fresh-frenquencies" ? "bg-primary/50" : ""
                }`}
              >
                <div
                  onClick={handleSidebarClick}
              ...
              >
                ...
                >
                  <Icons.flower className="h-5 w-5" />
                </div>
                <div
                  className={` ${
                    isSidebarOpen
              ...
              >
                ...
                >
                  <Icons.listMusic className="h-5 w-5" />
                </div>
                <div
                  className={` ${
                    isSidebarOpen
                    ...
                    {activeContent === 'country' && (
                      <div className="scrollable-container h-[68vh] min-w-[200px] border-l border-muted">
                        {countryOptions
                          .sort((a, b) => a.localeCompare(b))
                          .map((country) => (
                            <div key={country} className={`cursor-pointer py-2 pl-1.5 text-xs md:text-sm font-normal border-b border-muted hover:bg-muted/20 ${country === selectedCountry ? "text-texthigh bg-muted" : "text-textlow"}`} onClick={() => handleCountrySelection(country)}>
                              {country}
                            </div>
                          ))}
                      </div>
                    ...
                    {activeContent === 'year' && (
                      <div className="scrollable-container h-[68vh] min-w-[200px] border-l border-muted">
                        {yearOptions
                          ...
                          .map((year) => (
                            <div key={year} className={`cursor-pointer py-2 pl-1.5 text-xs md:text-sm font-normal border-b border-muted hover:bg-muted/20 ${year === selectedYear ? "text-texthigh bg-muted" : "text-textlow"}`} onClick={() => handleYearSelection(year)}>
                              {year}
                            </div>
                          ))}
                      </div>
          ...
          <MusicProvider selectedGenre={selectedGenre} selectedYear={selectedYear} selectedCountry={selectedCountry}>
