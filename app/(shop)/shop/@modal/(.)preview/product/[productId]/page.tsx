      ...
      <AlertDialogAction
        className={cn(
          buttonVariants({
            ...
            size: 'icon',
            className:
              'absolute right-14 top-4 h-auto w-auto shrink-0 rounded-sm bg-transparent px-2 py-1 text-foreground opacity-70 ring-offset-background transition-opacity hover:bg-transparent hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground',
          }),
        )}
        asChild
      >
        <Link href={`/shop/product/${product.id}/${product.name}`} replace>
          <Icon name="crop" className="h-4 w-4 " aria-hidden="true" />
        </Link>
      </AlertDialogAction>
